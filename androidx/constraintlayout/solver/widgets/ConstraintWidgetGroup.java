...
public class ConstraintWidgetGroup
{
  ...
  ConstraintWidgetGroup(List paramList, boolean paramBoolean)
  {
    mConstrainedGroup = paramList;
    mSkipSolver = paramBoolean;
  }
  
  private void getWidgetsToSolveTraversal(ArrayList paramArrayList, ConstraintWidget paramConstraintWidget)
  {
    if (mGroupsToSolver) {
      return;
    }
  ...
  private void updateResolvedDimension(ConstraintWidget paramConstraintWidget)
  {
    if (mOptimizerMeasurable)
    {
      ...
      if (mBaseline.mTarget != null)
      {
        ...
        i = mOwner.y + mOwner.mBaselineDistance - mBaselineDistance;
      ...
      if (localConstraintAnchor != null)
      {
        ...
        {
          j = mOwner.y + mOwner.getHeight();
        }
        else
        {
          j = i;
          if (mType == ConstraintAnchor.Type.MIDDLE) {
            j = mOwner.y;
          }
        }
      }
  ...
  public List getStartWidgets(int paramInt)
  {
    if (paramInt == 0) {
  ...
  Set getWidgetsToSet(int paramInt)
  {
    if (paramInt == 0) {
