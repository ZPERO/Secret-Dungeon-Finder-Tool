  ...
  extends FrameLayout
{
    ...
    if (collapsingTitleEnabled)
    {
      ...
      if (localObject != null)
      {
        ...
        if (drawCollapsingTitle)
        {
          ...
          ((CollapsingTextHelper)localObject).setCollapsedBounds(i1 + j, i2 + n + i3, i4 + k, tmpRect.bottom + n - toolbar.getTitleMarginBottom());
          localObject = collapsingTextHelper;
          if (i != 0) {
            j = expandedMarginEnd;
          } else {
    ...
    implements AppBarLayout.OnOffsetChangedListener
  {
    ...
    public void onOffsetChanged(AppBarLayout paramAppBarLayout, int paramInt)
    {
      ...
      int i;
      if (lastInsets != null) {
        i = this$0.lastInsets.getSystemWindowInsetTop();
      } else {
        i = 0;
      }
      paramAppBarLayout = this;
      ...
      while (j < k)
      {
        ...
        if (m != 1)
        {
          ...
          localViewOffsetHelper.setTopAndBottomOffset(MathUtils.clamp(-paramInt, 0, this$0.getMaxOffsetForPinChild((View)localObject2)));
        }
        j += 1;
      }
