  ...
  extends BaseContinuationImpl
{
  ...
  public ContinuationImpl(Continuation paramContinuation)
  {
    this(paramContinuation, localCoroutineContext);
  }
  
  public ContinuationImpl(Continuation paramContinuation, CoroutineContext paramCoroutineContext)
  {
    super(paramContinuation);
    _context = paramCoroutineContext;
  }
  ...
  {
    Object localObject = intercepted;
    if (localObject != null) {
      return localObject;
    }
    localObject = (ContinuationInterceptor)getContext().remove((CoroutineContext.Key)ContinuationInterceptor.this$0);
    if (localObject != null)
    {
      Continuation localContinuation = ((ContinuationInterceptor)localObject).interceptContinuation((Continuation)this);
    ...
    intercepted = ((Continuation)localObject);
    return localObject;
  }
  
  protected void releaseIntercepted()
  {
    Continuation localContinuation = intercepted;
    if ((localContinuation != null) && (localContinuation != (ContinuationImpl)this))
    {
      CoroutineContext.Element localElement = getContext().remove((CoroutineContext.Key)ContinuationInterceptor.this$0);
      if (localElement == null) {
        Intrinsics.throwNpe();
      }
      ((ContinuationInterceptor)localElement).releaseInterceptedContinuation(localContinuation);
    }
    intercepted = ((Continuation)CompletedContinuation.INSTANCE);
  }
}
